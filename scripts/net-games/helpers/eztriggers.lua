-- MODIFIED VERSION OF EZTRIGGERS TO REMOVE SOME DEPENDENCIES and unneeded code.
...
eztriggers.locks = {}
eztriggers.locks_by_player = {}


function eztriggers.get_lock(player_id,lock_id,timeout)
    --print(player_id,'trying to get lock ',lock_id)
    if eztriggers.locks[lock_id] == nil then
        if not eztriggers.locks_by_player[player_id] then
            eztriggers.locks_by_player[player_id] = {}
        end
        local lock = {
            release=function()
                --print(player_id,'released lock',lock_id)
                if eztriggers.locks_by_player[player_id] then
                    eztriggers.locks_by_player[player_id][lock_id] = nil
                end
                eztriggers.locks[lock_id] = nil
            end
        }
        eztriggers.locks_by_player[player_id][lock_id] = lock
        eztriggers.locks[lock_id] = lock
        if timeout then
    ...
    if emitter then
        ...
        if event_name then
            ...
            emitter:on("entered",function (event_info)
                ...
                if event then
                    local lock = eztriggers.get_lock(event_info.player_id, event_info.player_id..":"..event_name)
                    if lock then
...
Net:on("player_disconnect", function(event)
    ...
    if eztriggers.locks_by_player[event.player_id] then
        for lock_id, lock in pairs(eztriggers.locks_by_player[event.player_id]) do
            lock.release()
        end
    end
end)
