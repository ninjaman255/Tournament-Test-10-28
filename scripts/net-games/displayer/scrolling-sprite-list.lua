-- Scrolling Sprite List System
-- Debug is optional: set `_G.NG_SCROLL_LIST_DEBUG = true` somewhere during startup to enable logs.

ScrollingSpriteList = {}
ScrollingSpriteList.__index = ScrollingSpriteList

-- Debug toggles (default OFF)
ScrollingSpriteList.DEBUG = (_G.NG_SCROLL_LIST_DEBUG == true)

local function dprint(...)
  if ScrollingSpriteList.DEBUG then
    print(...)
  end
end

local _once = {}
local function dprint_once(key, ...)
  if not ScrollingSpriteList.DEBUG then return end
...
function ScrollingSpriteList:setupPlayerLists(player_id)
    if self.player_lists[player_id] then
        dprint("DEBUG: Player lists already set up for " .. player_id)
        return
    end
    ...
    dprint("DEBUG: Player lists setup for " .. player_id)
end

function ScrollingSpriteList:cleanupPlayerLists(player_id)
    local player_data = self.player_lists[player_id]
    if not player_data then
        dprint("DEBUG: cleanup called but no player data for " .. player_id)
        return
    end

    for list_id, _ in pairs(player_data.active_lists) do
...
function ScrollingSpriteList:createScrollingList(player_id, list_id, x, y, width, height, config)
    ...
    local player_data = self.player_lists[player_id]
    if not player_data then
        dprint_once("no_player_data:" .. player_id, "DEBUG: No player data for " .. player_id)
        return nil
    end
    ...
    list_config.list_id = list_id -- IMPORTANT: used by grid init for stable sprite ids
    ...
    -- Calculate bounds if backdrop is provided
    ...
    dprint("DEBUG: List config created with " .. #list_config.sprites .. " sprites")
    ...
    -- Pre-allocate sprites for this list (per-entry; safe + predictable)
    dprint("DEBUG: Pre-allocating sprites...")
    ...
    if list_config.backdrop then
        ...
        dprint("DEBUG: Backdrop drawn with ID: " .. tostring(list_data.backdrop_id))
    end
    ...
    dprint("DEBUG: Scrolling list created successfully")
    return list_id
end
...
function ScrollingSpriteList:preallocateSprites(player_id, list_id, config)
    for i, sprite_def in ipairs(config.sprites) do
        ...
        dprint("DEBUG: Pre-allocating sprite: " .. sprite_id)
        dprint("DEBUG: Texture path: " .. tostring(sprite_def.texture_path))
        dprint("DEBUG: Anim path: " .. tostring(sprite_def.anim_path))
        ...
        if success then
            dprint("DEBUG: Successfully allocated sprite: " .. sprite_id)
        else
            -- Choose your policy:
            -- debug-only:
            dprint("DEBUG: FAILED to allocate sprite " .. sprite_id .. ": " .. tostring(err))
            -- OR always-on warning (swap to print if you want):
            -- print("[ScrollingSpriteList] FAILED to allocate sprite " .. sprite_id .. ": " .. tostring(err))
        end
    end
end
...
function ScrollingSpriteList:initializeSpriteGrid(list_config)
    ...
    for _, sprite_def in ipairs(sprites) do
    ...
    local list_id = list_config.list_id or "list"
    for i, sprite_def in ipairs(sprites) do
        ...
        list_config.entry_states[i] = {
            ...
            sprite_id = list_id .. "_sprite_" .. i
        }
    end

    dprint("DEBUG: Grid initialized with " .. #sprites .. " entries, " .. actual_columns .. " columns")
end
...
function ScrollingSpriteList:drawListEntry(player_id, list_id, entry_index, list_data)
    ...
    if sprite_y + (entry_state.sprite_def.height or 10) >= config.bounds_top and
       sprite_y <= config.bounds_bottom then
        ...
        -- This can be noisy even in debug; keep it gated.
        ...
        if draw_success then
            -- (Intentionally no success print; it spams like crazy.)
            ...
            dprint("DEBUG: FAILED to draw sprite " .. sprite_id .. ": " .. tostring(draw_err))
        ...
        -- Out-of-bounds is normal in a scrolling list; do not log.
    end
end
...
function ScrollingSpriteList:drawErrorPlaceholder(player_id, display_id, x, y, z_order)
    ...
    if not success then
        dprint("DEBUG: Even error placeholder failed: " .. tostring(err))
        -- Or always-on:
        -- print("[ScrollingSpriteList] Even error placeholder failed: " .. tostring(err))
    end
end
